...
fn attachment_filename(part: &mail_parser::MessagePart<'_>) -> Option<String> {
...
fn content_type(part: &mail_parser::MessagePart<'_>) -> String {
    part.content_type()
        .and_then(|value| {
            value
                .subtype()
                .map(|subtype| format!("{}/{}", value.ctype(), subtype))
        })
...
pub fn parse_raw(raw: &[u8]) -> Option<ParsedInbound> {
    ...
    let bcc_addrs = addresses(msg.bcc());
    let reply_to_addrs = addresses(msg.reply_to());
    ...
    let headers = msg
        ...
        .filter_map(|(name, raw)| {
            raw.split_once(':')
                .map(|(_, value)| (name.to_ascii_lowercase(), value.trim().to_owned()))
        })
        .collect();
...
mod tests {
    ...
    #[test]
    fn attachment_preserves_filename_and_content_type() {
        // A multipart/mixed with one attachment part. The part metadata is part
        // of the Agent Mail attachment contract and must survive parsing.
        let raw = concat!(
            "From: a@x.com\r\n",
            "To: b@x.com\r\n",
            "Subject: WithAtt\r\n",
        ...
        assert_eq!(p.attachments[0].filename, "doc.pdf");
        assert_eq!(p.attachments[0].content_type, "application/pdf");
