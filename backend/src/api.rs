...
use super::{mime, EmailMessage, Inbox, InboxProvider};
use crate::MailState;

/// Max inbound body we accept (25 MiB).
const MAX_INBOUND_BYTES: usize = 26_214_400;

/// App event raised once a received message is durably stored.
///
/// Written out in full rather than built from [`crate::PLUGIN_ID`] because Core
/// matches the emit against the literal id in `apps-store/mail/manifest.json` —
/// grep-ability across the two files is worth more here than the concatenation.
const EVENT_MESSAGE_RECEIVED: &str = "@ryu/mail#message.received";
/// App event raised once an outbound message is accepted by the SMTP transport.
const EVENT_MESSAGE_SENT: &str = "@ryu/mail#message.sent";
...
#[openapi(
    ...
    components(schemas(CreateInboxBody, PatchInboxBody, SendBody))
)]
struct MailApiDoc;
                ...
                .emit_with_notify(
                    ...
                    Some(
                        ryu_app_events::NotifyHint::info(
                            "Message sent",
                            (!m.subject.is_empty()).then(|| m.subject.clone()),
                        )
                        .with_level("success"),
                    ),
                )
        ...
        Ok(m) => {
            ...
            state
                ...
                .emit_with_notify(
                    ...
                    // New inbound mail for an agent inbox is a "someone wrote in"
                    // moment the user is not watching for by definition.
...
mod openapi_tests {
    ...
    fn every_declared_route_appears_in_the_openapi_doc() {
            ...
            .expect("routes must be an array")
        {
            ...
            // `/inbound/:id` is EXEMPT ON PURPOSE, not overlooked. It is the per-inbox
