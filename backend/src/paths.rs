//! Inlined data-dir resolution (tracer copy of `apps/core/src/paths.rs`).
//!
//! The sidecar MUST resolve the SAME data dir Core uses so it opens the SAME
//! `mail.db`. The load-bearing rule is `RYU_DIR`-env-first: Core/Kernel passes
...
//!
//! Productionization follow-up: rather than re-deriving the path, Core could hand
//! the resolved data dir to the sidecar directly (one env/flag) — but re-deriving
//! keeps the sidecar self-sufficient with no coupling, which is the tracer's point.
